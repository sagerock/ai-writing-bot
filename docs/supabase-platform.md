...
| Audiences within a school | Library documents carry one tag: `staff`, `marketing`, `leadership`, `board`, `finance`. People hold a set per school in `romalume.member_audiences`. Enforced in Postgres by `romalume.can_read_audience()`; Qdrant carries the same tag and pre-filters. Marketing was added as a first-class audience on 2026-09-22. |
| RomaLume administration | `sage@sagerock.com` is the sole RomaLume administrator. Sage can preview any enabled client workspace under a staff, marketing, leadership, board, finance, or full-access profile. The preview changes branding and shared-library retrieval, but retains Sage's user id so it never opens another person's private chats or drafts. |
